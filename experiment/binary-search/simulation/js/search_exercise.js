...
function init() {
  ...
  document.getElementById("reset").onclick = function () {
    ...
    if (queryField.value.trim() === "" || isNaN(queryField.value)) {
      writeInstructionMessage(
        "Please enter a valid integer query element before starting the binary search.",
      );
      return;
    }
    this.innerHTML = "Reset";
    searchArt.queryElement = Number(queryField.value);
    document.getElementById("reset").disabled = true;
    getBinarySearchOrder(searchArt.randomNumberArray, searchArt.queryElement);
    writeInstructionMessage(
      `Current Start Index = ${searchArt.start} and Current End Index = ${searchArt.end}. Click on the Middle element of this Array window to go to check if it matches with the query element!`,
    );
  ...
  writeInstructionMessage(
    "Enter a query element to be searched in the array and press start to begin binary searching on the elements!",
  );
}
function reload() {
...
function getBinarySearchOrder() {
  ...
  while (start <= end) {
    ...
    if (searchArt.randomNumberArray[mid] === searchArt.queryElement)
      return true;
...
function checkAnswer(val) {
  if (val == searchArt.midElementIndexOrder[0]) {
    ...
    if (searchArt.midElementIndexOrder.length > 1) {
      if (
        searchArt.midElementIndexOrder[1] < searchArt.midElementIndexOrder[0]
      ) {
        ...
        writeInstructionMessage(
          `Good Job! Since ${searchArt.randomNumberArray[searchArt.mid]} is greater than ${searchArt.queryElement}, we ignore the right half of the current window. Current Start Index = ${searchArt.start} and Current End Index = ${searchArt.end}. Pick the middle element of this window.`,
        );
      } else {
        ...
        writeInstructionMessage(
          `Good Job! Since ${searchArt.randomNumberArray[searchArt.mid]} is lesser than ${searchArt.queryElement}, we ignore the left half of the current window. Current Start Index = ${searchArt.start} and Current End Index = ${searchArt.end}. Pick the middle element of this window.`,
        );
      }
    } else {
      // last element could result in match found or match not found
      if (
        searchArt.randomNumberArray[searchArt.mid] == searchArt.queryElement
      ) {
        // Element found in the array
        writeInstructionMessage(
          `${searchArt.queryElement} was found in the array at position ${searchArt.mid}. Binary Search Complete. Great Work!`,
        );
        ...
        writeInstructionMessage(
          `${searchArt.queryElement} was not found in the array. Binary Search Complete. Great Work!`,
        );
      }
      document.getElementById("reset").disabled = false;
    }
    ...
    writeInstructionMessage(
      `Wrong Element picked. Current Start Index = ${searchArt.start} and Current End Index = ${searchArt.end}. Pick the middle element of this window.`,
    );
...
function manage(txt) {
  ...
  bt.disabled = false;
  if (txt.value.trim() !== "" && isNaN(txt.value)) {
    writeInstructionMessage(
      "Please enter an integer only for the query element.",
    );
    txt.value = "";
  }
}
